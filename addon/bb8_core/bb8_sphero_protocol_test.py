...
SPHERO_COMMAND_UUID = (
    "22bb746f-2ba1-7554-2d6f-726568705327"  # TX (write to robot)
)
SPHERO_RESPONSE_UUID = (
    "22bb746f-2ba6-7554-2d6f-726568705327"  # RX (read from robot)
)
    ...
    try:
        async with BleakClient(BB8_PRIMARY["address"], timeout=15.0) as client:
            ...
            await client.write_gatt_char(
                command_char, wake_packet, response=False
            )
            ...
            # Green LED
            led_green = create_sphero_packet(
                0x00, 0x20, 0x03, bytes([0, 255, 0, 0])
            )
            print("   🟢 Green LED")
            await client.write_gatt_char(
                command_char, led_green, response=False
            )
            await asyncio.sleep(1.0)

            # Blue LED
            led_blue = create_sphero_packet(
                0x00, 0x20, 0x04, bytes([0, 0, 255, 0])
            )
            print("   🔵 Blue LED")
            ...
            print("   ⚪ White LED")
            await client.write_gatt_char(
                command_char, led_white, response=False
            )
            await asyncio.sleep(1.5)

            # LED Off
            led_off = create_sphero_packet(
                0x00, 0x20, 0x06, bytes([0, 0, 0, 0])
            )
            print("   ⚫ LED Off")
            ...
            for i, wake_bytes in enumerate(simple_wakes):
                ...
                await client.write_gatt_char(
                    command_char, wake_bytes, response=False
                )
            ...
            for _ in range(3):
                ...
                await client.write_gatt_char(
                    command_char, led_off, response=False
                )
                await asyncio.sleep(0.3)
...
async def main():
    ...
    if success:
        ...
        response = (
            input("🤔 Any response observed this time? (y/n): ").lower().strip()
        )
        ...
        if response.startswith("y"):
            ...
            print(
                "   1. BB-8 may need to be 'awakened' by physical interaction first"
            )
            ...
            if different_char.startswith("y"):
                print(
                    "💡 Next test should try all 11 characteristics systematically"
                )
    else:
