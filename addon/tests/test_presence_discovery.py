...
_MODULE_SPEC = importlib.util.spec_from_file_location(
    "int03_presence_discovery_test_module",
    _MODULE_PATH,
)
...
def _presence_payload() -> dict:
    _, payload_json = presence_discovery_config()
    return json.loads(payload_json)


def test_presence_discovery_topic():
    topic, _ = presence_discovery_config()

    assert topic == "homeassistant/binary_sensor/bb8_presence/config"


def test_presence_discovery_device_class():
    payload = _presence_payload()

    assert payload["device_class"] == "presence"


def test_presence_discovery_state_topic():
    payload = _presence_payload()

    assert payload["state_topic"] == "bb8/state/presence"


def test_presence_discovery_payload_on_off():
    payload = _presence_payload()

    assert payload["payload_on"] == "detected"
    assert payload["payload_off"] == "not_detected"


def test_presence_discovery_no_availability():
    payload = _presence_payload()
...
def test_presence_discovery_device_info():
    payload = _presence_payload()
...
def test_presence_discovery_unique_id_distinct():
    payload = _presence_payload()
    _, light_payload_json = light_discovery_config()
    ...
    assert payload["unique_id"] == "bb8_ble_presence"
...
def test_presence_discovery_valid_json():
    _, payload_json = presence_discovery_config()

    payload = json.loads(payload_json)

    assert isinstance(payload, dict)
    assert payload["name"] == "BB-8 Presence"
    assert payload["object_id"] == "bb8_presence"
