...
class FakeClient:
    def __init__(self): self.subs=[]; self.pubs=[]; self.reconnects=0
    def subscribe(self, topic, qos=0): self.subs.append((topic,qos)); return (1,0)  # rc=1 -> error path
...
def test_ack_paths_retain_qos_and_empty_payload_handling():
    ...
    md.on_message(c, None, types.SimpleNamespace(topic="bb8/echo/cmd", payload=b""))
    # good payload → first publish rc=0, second forced rc!=0 (graceful)
    msg = types.SimpleNamespace(topic="bb8/echo/cmd",
