...
- **Runtime Clone (HA mount):** `/Volumes/HA/addons/local/beep_boop_bb8` — Not a source of truth except for explicit one-time imports.
...
3. Sync runtime:
	...
	git -C /Volumes/HA/addons/local/beep_boop_bb8 checkout -B main origin/main
	git -C /Volumes/HA/addons/local/beep_boop_bb8 reset --hard origin/main
	```
4. Emit tokens:
	```bash
	WSH=$(git -C /Users/evertappels/Projects/HA-BB8/addon rev-parse --short HEAD)
	RTH=$(git -C /Volumes/HA/addons/local/beep_boop_bb8 rev-parse --short HEAD)
...
git -C /Volumes/HA/addons/local/beep_boop_bb8 fetch --all --prune
git -C /Volumes/HA/addons/local/beep_boop_bb8 reset --hard origin/main
git -C /Volumes/HA/addons/local/beep_boop_bb8 clean -fdX
```

**Exception (accept runtime edits as canonical one-time import):**
```bash
git -C /Volumes/HA/addons/local/beep_boop_bb8 add -A
git -C /Volumes/HA/addons/local/beep_boop_bb8 commit -m "hotfix: accept runtime edits as canonical"
git -C /Volumes/HA/addons/local/beep_boop_bb8 push origin HEAD:main
...
for d in docs ops reports scripts tools addon .github; do test ! -d /Volumes/HA/addons/local/beep_boop_bb8/$d; done && echo "[runtime ok]"
