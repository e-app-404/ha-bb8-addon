...
Below are some examples of how the `hass` object is accessed in different contexts:
...
def setup(hass, config):
  ...
  hass.states.set("example.entity", "on")
  return True
```
...
def setup_platform(hass, config, add_entities, discovery_info=None):
  # Use hass to add entities or interact with the system
  add_entities([MyEntity(hass)])
```
...
class MyEntity(Entity):
  ...
  def turn_on(self):
    # Use self.hass to interact with Home Assistant
    self.hass.states.set(self.entity_id, "on")
```

> **Note**: In asynchronous integrations, use `async_setup` and `async_setup_platform` with `async`/`await` syntax for non-blocking operations.

For more details, refer to the [Home Assistant developer documentation](https://developers.home-assistant.io/docs/dev_101_index).
