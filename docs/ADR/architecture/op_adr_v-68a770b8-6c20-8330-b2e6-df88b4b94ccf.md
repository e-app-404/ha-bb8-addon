...
ha addons rebuild succeeded in LOCAL_DEV; PUBLISH rebuild refused or failed if image was missing.
...
Controller exited; s6 restarted rapidly.

**Investigation Method**:

Log pattern analysis; verifying importability; changing wrapper behavior.
...
**Positive**: Stable, contract-compliant single start with supervisor holding the service.

**Negative**: None observed.

**Unknown/Untested**: None.
...
**Positive**: Confirmed end-to-end MQTT path; telemetry emitted.

**Negative**: Deprecation warning:
...
## Consequences
**Positive**: Predictable load; safer steady state.

**Negative**: Manual enable/disable step for tests.

**Unknown/Untested**: Long-term memory profiling with echo enabled continuously.
