...
class TestMQTTDiscoveryOwnership:
    ...
    def test_single_discovery_owner(self, mock_mqtt_client, mock_facade, mock_config):
        ...
        with patch("addon.bb8_core.mqtt_dispatcher.threading.Event") as mock_event:
            ...
            published_messages.append(
                (f"{base_topic}/presence/config", json.dumps(discovery_payload))
            )
        ...
        assert len(device_blocks) <= 1, (
            "Multiple device blocks found - ownership conflict"
        )
    ...
    def test_restart_persistence(self, mock_mqtt_client, mock_facade, mock_config):
        ...
        assert hasattr(mock_client, "publish"), (
            "MQTT client should support state publishing"
        )
        assert hasattr(mock_client, "reconnect"), (
            "MQTT client should support reconnection"
        )
