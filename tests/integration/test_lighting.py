...
class TestLightingController:
    ...
    async def test_animation_cancellation_speed(self, lighting_controller, mock_toy):
        ...
        preset_task = asyncio.create_task(
            lighting_controller.run_preset("police")
        )
        ...
        assert cancel_time <= 0.1, f"Cancellation took {cancel_time:.3f}s, should be ≤0.1s"
    ...
    async def test_multiple_preset_cancellation(self, lighting_controller, mock_toy):
        ...
        # Start sunset preset
        sunset_task = asyncio.create_task(
            lighting_controller.run_preset("sunset")
        )
        await asyncio.sleep(0.01)

        # Start police preset (should cancel sunset)
        start_time = time.time()
        police_task = asyncio.create_task(
            lighting_controller.run_preset("police")
        )
        await asyncio.sleep(0.01)
        cancel_time = time.time() - start_time
    ...
    async def test_cancel_active_method(self, lighting_controller):
        ...
        for preset in ["police", "sunset"]:
            task = asyncio.create_task(
                lighting_controller.run_preset(preset)
            )
            tasks.append(task)
            await asyncio.sleep(0.01)
    ...
    @pytest.mark.asyncio
    async def test_toy_disconnected_graceful_failure(self, lighting_controller, mock_ble_session):
        ...
        assert success, "Preset should succeed even when disconnected (for state management)"
    ...
    async def test_concurrent_led_commands(self, lighting_controller, mock_toy):
        ...
        assert mock_toy.set_led.call_count >= 1, "At least one LED command should execute"
