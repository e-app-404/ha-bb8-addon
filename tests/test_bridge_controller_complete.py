...
class TestBridgeControllerComplete:
    ...
    @pytest.mark.skip(
        reason="Test depends on global state that may be modified by other tests"
    )
    ...
    @pytest.mark.skip(
        reason="Test uses deprecated _BRIDGE global that no longer exists"
    )
    ...
    @pytest.mark.skip(
        reason="Comprehensive integration test uses multiple deprecated APIs"
    )
    def test_integration_end_to_end_flow(self):
        """Test comprehensive end-to-end integration flow."""
        with patch.multiple(
            "addon.bb8_core.bridge_controller.load_config"
        ) as mock_load_config:
            mock_load_config.return_value = (self.mock_config, "/test/config.yaml")
