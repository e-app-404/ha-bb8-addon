...
class BleSession:
    ...
    async def connect(self, mac: str | None = None) -> None:
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                ...
                try:
                    ...
                    self._last_connect_time = connect_end - self._connect_start_time
        ...
        raise ConnectionError(
            f"Failed to connect after {self._max_attempts} attempts: {last_error}"
        )
    ...
    async def roll(self, speed: int, heading: int, ms: int | None = None) -> None:
    ...
    async def _roll_impl(self, speed: int, heading: int, ms: int | None) -> None:
