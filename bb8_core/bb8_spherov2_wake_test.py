...
async def test_spherov2_wake():
    ...
    def analyze_signal_change(baseline, post, device_name):
        if baseline and post:
            baseline_avg = sum(reading[1] for reading in baseline) / len(
                baseline
            )
    ...
    if abs(bb54a_change) > 4 or abs(s33_change) > 4:
        ...
        if bb54a_change > 4:
            print(
                "📡 BB-B54A showed strongest response - primary wake interface"
            )
        if s33_change > 4:
            print(
                "📡 S33 BB84 LE showed strongest response - operational interface"
            )
