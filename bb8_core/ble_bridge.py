...
logger.info(
    {"event": "version_probe", "bleak": bleak_version, "spherov2": spherov2_version}
)
...
class BLEBridge:
    ...
    def drive(self, heading: int, speed: int) -> None:
        ...
        try:
            logger.info(
                {"event": "ble_drive", "heading": int(heading), "speed": int(speed)}
            )
            ...
            if hasattr(self, "_mqtt") and self._mqtt.get("client"):
                echo_scalar(
                    self._mqtt["client"], self._mqtt["base"], "heading", int(heading)
                )
                echo_scalar(
                    self._mqtt["client"], self._mqtt["base"], "speed", int(speed)
                )
        except Exception as e:
    ...
    def sleep(self, after_ms: int | None = None) -> None:
        ...
        with self._lock:
            try:
                ...
                logger.error(
                    {"event": "ble_sleep_error", "after_ms": after_ms, "error": repr(e)}
                )
        ...
        def _handle_power(_client, _userdata, msg):
            try:
                ...
                if val == "ON":
                    ...
                    logger.warning({"event": "ble_cmd_power_invalid", "payload": val})
            except Exception as e:
                logger.error({"event": "ble_cmd_power_handler_error", "error": repr(e)})
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt < max_attempts:
                ...
                publish_bb8_error(
                    f"{cmd_func.__name__} failed after {max_attempts} attempts: {e}"
                )
    return None
