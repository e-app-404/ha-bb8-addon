...
if [ ! -f ops/validators/guard_size_symlinks.sh ]; then
...
# Local pre-commit hook (pointing to ops path)
...
# CI workflow (idempotent overwrite) references ops/validators
...
git add -- ops/validators/guard_size_symlinks.sh .github/workflows/guard-size-symlinks.yml

if git diff --cached --quiet; then
  echo "Nothing to commit (files already up to date)."
else
  git commit -m "guardrails(v2.1): filename-safe diff capture; clean tree guard; CI workflow (ops/)"
fi
