...
python ops/evidence/mqtt_probe.py --timeout 8 --require-echo "$REQUIRE_DEVICE_ECHO" 2>&1 | tee -a "$RUNLOG" || true

echo "[evidence] step=capture" | tee -a "$RUNLOG"
python ops/evidence/capture_trace.py --duration 12 --out "$TRACE_JSONL" 2>&1 | tee -a "$RUNLOG" || true
...
if python ops/evidence/collect_stp4.py 2>&1 | tee -a "$RUNLOG"; then
  :
else
