...
class MockBB8Router:
    ...
    def run_comprehensive_tests(self):
        ...
        logger.info("Starting comprehensive BB-8 command validation tests (mock)")
        ...
        test_cases = [
            ...
            ('drive', {'speed': 300, 'heading': 400, 'ms': 10000, 'cid': 'clamp_test'}),
            ...
            ('drive', {'speed': 100, 'heading': 90, 'ms': 2000, 'turbo': True, 'cid': 'extra_fields'}),  # Extra fields
            ('stop', {'cid': 'invalid@cid#format!'}),  # Invalid correlation ID
            
            # Emergency stop sequence
            ('drive', {'speed': 50, 'heading': 0, 'ms': 1000, 'cid': 'blocked_by_estop'}),  # Should be blocked
            ('drive', {'speed': 50, 'heading': 180, 'ms': 1000, 'cid': 'after_estop_clear'}),  # Should work after clear
    ...
    def generate_report(self) -> str:
        ...
        # Summary statistics
        valid_count = sum(1 for r in self.test_results if r['validation_result']['valid'])
        invalid_count = sum(1 for r in self.test_results if not r['validation_result']['valid'])
        ...
        report_lines.extend([
            ...
            *[f"  {cmd}: {count} tests" for cmd, count in sorted(cmd_types.items())],
            ""
        ])
        ...
        for i, result in enumerate(self.test_results, 1):
            status = "VALID" if result['validation_result']['valid'] else "REJECTED"
            ...
            if result['processed_payload'] != result['original_payload']:
                report_lines.append(f"  Processed: {json.dumps(result['processed_payload'])}")
            
            if result['validation_result']['message']:
                report_lines.append(f"  Message: {result['validation_result']['message']}")
