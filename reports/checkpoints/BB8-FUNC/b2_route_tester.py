...
class BB8RouteTestHarness:
    ...
    def __init__(self, host: str = 'core-mosquitto', port: int = 1883, 
                 username: str = 'mqtt_bb8', password: str = 'mqtt_bb8'):
        self.host = host
        self.port = port
    ...
    def publish_command(self, command: str, payload: Dict[str, Any], 
                       expect_ack: bool = True) -> Dict[str, Any]:
        """
        ...
        is_valid, processed_payload, validation_msg = self.validator.validate_command(topic, payload)
        ...
        try:
            ...
            if expect_ack:
                    ...
                    with self.lock:
                        if ack_topic in self.received_acks:
                            ...
                            if ack_cid == cid:
                                ...
                                logger.warning(f"Correlation ID mismatch: sent {cid}, received {ack_cid}")
                            ...
                            if is_valid and ack_ok:
                                ...
                                test_result['test_status'] = 'correctly_rejected'
                            else:
                ...
                if not test_result['ack_received']:
                    ...
                    logger.warning(f"No ack received for {topic} within {timeout}s")
    ...
    def run_comprehensive_tests(self) -> List[Dict[str, Any]]:
        ...
        self.publish_command('clear_estop', {
            'cid': 'clear_emergency'
        })
    ...
    def generate_report(self) -> str:
        ...
        # Summary statistics
        success_count = sum(1 for r in self.test_results if r['test_status'] == 'success')
        rejected_count = sum(1 for r in self.test_results if r['test_status'] == 'correctly_rejected')
        timeout_count = sum(1 for r in self.test_results if r['test_status'] == 'ack_timeout')
        failed_count = sum(1 for r in self.test_results if r['test_status'] in ['status_mismatch', 'publish_failed'])
        ...
        for i, result in enumerate(self.test_results, 1):
            ...
            if result['validation_local']['message']:
                report_lines.append(f"  Validation Message: {result['validation_local']['message']}")
            ...
            if 'publish_error' in result:
                report_lines.append(f"  Publish Error: {result['publish_error']}")
...
def main():
    ...
    try:
        ...
        failed_tests = [r for r in results if r['test_status'] in ['status_mismatch', 'publish_failed', 'ack_timeout']]
        if failed_tests:
