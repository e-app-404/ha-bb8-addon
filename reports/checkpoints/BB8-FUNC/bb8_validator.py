...
class BB8CommandValidator:
    ...
    def clamp_value(
        self, value: Any, field_name: str, min_val: int, max_val: int
    ...
    def validate_command(
        ...
        if topic == "bb8/cmd/drive" and self.estop_active:
            return (
                False,
                payload,
                "motion commands blocked - emergency stop active",
            )
        ...
        try:
            ...
            if topic == "bb8/cmd/drive":
                if "speed" in processed:
                    processed["speed"], warning = self.clamp_value(
                        processed["speed"], "speed", 0, 255
                    )
                    if warning:
                        warnings.append(warning)

                if "heading" in processed:
                    processed["heading"], warning = self.clamp_value(
                        processed["heading"], "heading", 0, 359
                    )
                    if warning:
                        warnings.append(warning)

                if "ms" in processed:
                    processed["ms"], warning = self.clamp_value(
                        processed["ms"], "ms", 0, 5000
                    )
                    if warning:
                        warnings.append(warning)
                ...
                for color in ["r", "g", "b"]:
                    if color in processed:
                        processed[color], warning = self.clamp_value(
                            processed[color], color, 0, 255
                        )
                        if warning:
                            warnings.append(warning)
            ...
            if topic == "bb8/cmd/estop":
                ...
                warnings.append(
                    "emergency stop activated - all motion disabled"
                )
                ...
                if not self.estop_active:
                    return (
                        False,
                        processed,
                        "cannot clear estop - no active emergency stop",
                    )
                self.estop_active = False
    ...
    def create_ack(
        ...
        ack = {"ok": success, "timestamp": datetime.utcnow().isoformat() + "Z"}
...
def test_validator():
    ...
    try:
        ...
        is_valid, processed, error = validator.validate_command(
            "bb8/cmd/drive", valid_cmd
        )
        ...
        is_valid, processed, error = validator.validate_command(
            "bb8/cmd/drive", clamp_cmd
        )
        print(
            f"Clamped command: {is_valid}, Processed: {processed}, Warning: {error}"
        )

        # Test missing field
        invalid_cmd = {"speed": 100, "heading": 90}  # missing 'ms'
        is_valid, processed, error = validator.validate_command(
            "bb8/cmd/drive", invalid_cmd
        )
