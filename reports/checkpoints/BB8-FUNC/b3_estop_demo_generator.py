...
async def demo_emergency_stop_sequence():
    ...
    try:
        ...
        logger.log(
            f"✓ Drive command validated: speed={speed}, heading={heading}, duration={duration}ms"
        )
    ...
    try:
        ...
        logger.log(
            f"✓ Second drive command validated: speed={speed}, heading={heading}, duration={duration}ms"
        )
    except Exception as e:
    ...
    logger.log(
        f"  Status: {'ACTIVE' if safety.is_estop_active() else 'INACTIVE'}"
    )
    ...
    try:
        ...
        logger.log(
            f"✗ UNEXPECTED: Drive command succeeded during estop: speed={speed}, heading={heading}, duration={duration}ms",
            "ERROR",
        )
    except Exception as e:
    ...
    try:
        ...
        logger.log(
            f"✗ UNEXPECTED: Second drive command succeeded during estop",
            "ERROR",
        )
    except Exception as e:
        logger.log(
            f"✓ Second drive command correctly blocked during estop: {e}"
        )
    ...
    try:
        ...
        logger.log(
            f"✓ Drive command after estop clear: speed={speed}, heading={heading}, duration={duration}ms"
        )
    except Exception as e:
    ...
    try:
        ...
        logger.log(
            f"✓ Final drive command validated: speed={speed}, heading={heading}, duration={duration}ms"
        )
    except Exception as e:
    ...
    try:
        speed, heading, duration = safety.validate_drive_command(
            250, 90, 1000
        )  # Over limit
        logger.log(
            f"✓ Speed clamping test: 250 → {speed} (max: {config.max_drive_speed})"
        )
    except Exception as e:
    ...
    try:
        ...
        logger.log(
            f"✓ Duration clamping test: 3500ms → {duration}ms (max: {config.max_drive_duration_ms}ms)"
        )
    except Exception as e:
    ...
    try:
        safety.validate_drive_command(50, 45, 500)
        logger.log(
            "✗ UNEXPECTED: Second rapid command succeeded (should be rate limited)",
            "ERROR",
        )
    except Exception as e:
        logger.log(
            f"✓ Second rapid command correctly rate limited: Rate limit protection working"
        )
    ...
    logger.log(
        f"  Emergency Stop: {'ACTIVE' if status['estop_active'] else 'INACTIVE'}"
    )
...
async def demo_facade_integration():
    ...
    with open(log_file, "a") as f:
        ...
        f.write(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: Testing facade-level emergency stop integration\n"
        )
        ...
        f.write(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: Mock facade initialized\n"
        )
        ...
        try:
            ...
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: ✓ Facade drive command accepted\n"
            )
        except Exception as e:
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] ERROR: ✗ Facade drive command failed: {e}\n"
            )
        ...
        try:
            ...
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: ✓ Facade emergency stop activated\n"
            )
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO:   Status: {'ACTIVE' if facade._safety.is_estop_active() else 'INACTIVE'}\n"
            )
        except Exception as e:
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] ERROR: ✗ Facade estop failed: {e}\n"
            )
        ...
        try:
            ...
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] ERROR: ✗ UNEXPECTED: Drive command succeeded during estop\n"
            )
        except Exception as e:
            # This should happen due to safety validation
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: ✓ Drive command correctly blocked during estop\n"
            )
        ...
        try:
            facade.clear_estop()
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: ✓ Facade emergency stop cleared\n"
            )
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO:   Status: {'ACTIVE' if facade._safety.is_estop_active() else 'INACTIVE'}\n"
            )
            ...
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] ERROR: ✗ Facade clear estop failed: {e}\n"
            )
        ...
        try:
            ...
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: ✓ Drive command after estop clear accepted\n"
            )
        except Exception as e:
            f.write(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] ERROR: ✗ Drive command after estop clear failed: {e}\n"
            )

        f.write(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}] INFO: === FACADE INTEGRATION DEMO COMPLETE ===\n"
        )
